    ...
    <div className="mx-auto px-2 sm:px-0">
      <div className={`grid grid-cols-1 gap-4 sm:gap-6 ${columns === 2 ? 'lg:grid-cols-2' : ''}`}>
        ...
        {metadata.enable_evm && (
          <div>
            <div className="heima-panel rounded-lg p-3 sm:p-5 mb-4 sm:mb-5">
              <div className="mb-3 sm:mb-4 text-base sm:text-lg font-semibold">EVM Contract</div>
              <div className={`grid gap-4 mb-4 ${columns === 2 ? 'grid-cols-2' : 'grid-cols-4'}`}>
                <InfoCard title="EVM Contract Block" value={formatNumber(metadata.finalized_blockNum)} href="/block" />
                <InfoCard title="Transaction" value={formatNumber(metadata.total_transaction)} href="/tx" />
                <InfoCard title="EVM Account" value={formatNumber(metadata.total_evm_account)} href="/address" />
                <InfoCard title="EVM Contract" value={formatNumber(metadata.total_evm_contract)} href="/contract" />
              </div>
            </div>
            <div className={`grid gap-4 grid-cols-1 ${columns === 1 ? 'lg:grid-cols-2' : ''}`}>
              {columns === 1 && (
